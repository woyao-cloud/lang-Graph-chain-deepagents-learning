"""
deepagents run 命令 - 启动执行
用法: deepagents run --phase <plan|execute> [--parallel] [--watch] [--resume]
"""
...
def run(
    ...
    from deepagents.agents.supervisor import SupervisorAgent
    ...
    if not os.path.exists("workflow.md"):
        typer.echo("❌ workflow.md 不存在，请先运行 deepagents init")
        raise typer.Exit(1)
    ...
    if phase == "plan":
        ...
        typer.echo("   运行 deepagents confirm 确认规划")
